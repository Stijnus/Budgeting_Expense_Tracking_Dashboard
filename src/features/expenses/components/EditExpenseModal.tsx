...
async function updateTags(
  ...
  const newTagNames = newTagString
    .split(",")
  ...
  currentLinks?.forEach((link) => {
    const tagLink = link as {
      tag_id: string;
      tags?: { id: string; name: string };
    };
    if (tagLink.tags) {
      // Check if tags relation exists
      currentTagsMap.set(tagLink.tag_id, tagLink.tags.name);
    }
  });
  const currentTagNames = Array.from(currentTagsMap.values());

  // 2. Determine tags to add and remove
  const tagsToAdd = uniqueNewTagNames.filter(
    (name) => !currentTagNames.includes(name)
  );
  const tagsToRemove = Array.from(currentTagsMap.entries()) // Get [tagId, tagName] pairs
    .filter(([, name]) => !uniqueNewTagNames.includes(name))
    .map(([tagId]) => tagId); // Get only the IDs of tags to remove
...
export default function EditExpenseModal({
      ...
      const fetchCurrentTags = async () => {
        ...
        try {
          ...
          const tagNames =
            data
              ?.map((item) => {
                const tagItem = item as { tags?: { name: string } };
                return tagItem.tags?.name;
              })
              .filter(Boolean) || [];
          ...
          setError(`Could not load current tags: ${errorMessage}`); // Show error in modal
        } finally {
  ...
  const handleSave = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const {
        data: { user },
      } = await supabase.auth.getUser(); // Get user for tag handling
      if (!user) throw new Error("User not found for tag update.");
      const tagErrors = await updateTags(user.id, expense.id, tags);

      if (tagErrors.length > 0) {
        setError(
          `Expense updated, but failed to process some tags: ${tagErrors.join(
            ", "
          )}`
        );
      } else {
  ...
  return (
    ...
    >
      <div
        className={`bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4 relative transform transition-all duration-300 ${
