...
type SortColumn = "date" | "amount" | "description" | "categories";
...
export default function ExpenseList({
  ...
  const fetchExpensesAndCategories = useCallback(async () => {
    ...
    try {
      ...
      if (filterEndDate) {
        ...
        query = query.lte("date", endDatePlusOne.toISOString().split("T")[0]);
      }
      ...
      if (sortColumn === "categories")
        effectiveSortColumn = "date"; // Fallback category sort
  ...
  const handleSort = (column: SortColumn) => {
    if (sortColumn === column)
      ...
      setSortDirection(["date", "amount"].includes(column) ? "desc" : "asc");
    }
  };
  ...
  const renderSortIcon = (column: SortColumn) => {
    if (sortColumn !== column)
      return (
        <ArrowUpDown size={14} className="ml-1 text-gray-400 inline-block" />
      );
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const { data: allExpenses, error: fetchError } = await supabase
        ...
        .eq("user_id", userId)
        .eq("type", "EXPENSE")
        .order("date", { ascending: false }); // Order by date descending for export
      ...
      allExpenses.forEach((exp) => {
        const row = [
          ...
          escapeCsvValue(exp.categories?.name || "Uncategorized"), // Access joined category name
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      setExportError(`Failed to export data: ${errorMessage}`);
    } finally {
      setIsExporting(false);
    }
  };
      ...
      <div className="p-4 bg-white rounded-lg shadow">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-10 text-gray-500">
            <Loader2 className="animate-spin h-8 w-8 mr-3" /> Loading
            expenses...
          </div>
        ) : expenses.length === 0 ? (
          <div className="text-center py-10 px-4">
            ...
            {(filterStartDate ||
              ...
              <div className="mt-6">
                ...
                >
                  <FilterX size={16} className="-ml-1 mr-2 h-5 w-5" /> Clear
                  Filters & Search
                </button>
              </div>
            )}
