...
import { Loader2, AlertTriangle, Trash2, Pencil, Save, XCircle, PlusCircle, Target } from 'lucide-react';
...
export default function BudgetManager() {
    ...
    const [currentBudget, setCurrentBudget] = useState<BudgetWithDetails | null>(null); // Use extended type
    ...
    const fetchBudgetData = useCallback(async () => {
        ...
        try {
            ...
            const minStartDate = fetchedBudgets.reduce((min, b) => b.start_date < min ? b.start_date : min, fetchedBudgets[0].start_date);
            const maxEndDate = fetchedBudgets.reduce((max, b) => b.end_date > max ? b.end_date : max, fetchedBudgets[0].end_date);
            ...
            const budgetsWithDetails = fetchedBudgets.map(budget => {
                const spent = relevantExpenses
                    .filter(expense => {
                        ...
                        const isMatchingCategory = budget.category_id === null || budget.category_id === expense.category_id;
                ...
                return {
                    ...
                    category_name: (budget as any).categories?.name || null // Get category name from join
                };
            });
    ...
    const handleDeleteClick = async (budgetId: string) => {
        ...
        try {
            ...
            setFormError(`Failed to delete budget: ${err.message}`);
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
                ...
                // Insert new budget
                 ...
                 if (insertError) {
                    if (insertError.code === '23505') {
                        throw new Error('A budget for this category/period already exists.');
                    }
                    throw insertError;
                 }
            }
            ...
            setFormError(`Failed to save budget: ${err.message}`);
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const calculateProgress = (budget: BudgetWithDetails) => {
        ...
        const percentage = budget.amount > 0 ? Math.min(100, (budget.spent / budget.amount) * 100) : 0; // Cap at 100%
    ...
    return (
        <div className="p-4 bg-white rounded-lg shadow">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">{isEditing ? 'Edit Budget' : 'Add New Budget'}</h2>

            {/* Add/Edit Budget Form */}
            <form onSubmit={handleSubmit} className="mb-6 p-4 border border-gray-200 rounded-md bg-gray-50 space-y-4">
                {/* Category Selection */}
                <div>
                    ...
                    {categories.length === 0 && !loading && (
                        <p className="mt-1 text-xs text-gray-500">No categories found. Add categories first.</p>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="budget-start-date" className="block text-sm font-medium text-gray-700">Start Date *</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label htmlFor="budget-end-date" className="block text-sm font-medium text-gray-700">End Date *</label>
                        <input
                            type="date"
                            id="budget-end-date"
                            value={endDate}
                ...
                <div className="flex items-center justify-end space-x-3">
                    ...
                    <button
                        type="submit"
                        disabled={isSubmitting || loading}
                        className={`px-4 py-2 text-sm font-medium text-white border border-transparent rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 flex items-center justify-center min-w-[100px] ${isEditing ? 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500' : 'bg-green-600 hover:bg-green-700 focus:ring-green-500'}`}
                    >
                        {isSubmitting ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : (isEditing ? <Save size={16} className="mr-2"/> : <PlusCircle size={16} className="mr-2"/>)}
            ...
            <h3 className="text-lg font-semibold mb-3 text-gray-700 mt-6 pt-4 border-t">Existing Budgets</h3>
            <div className="min-h-[100px]">
                {loading ? (
                    ...
                    <div className="text-center py-6 px-4 text-gray-500">
                        ...
                        <p className="text-xs">Use the form above to add your first budget goal.</p>
                    </div>
                ) : (
                    <ul className="space-y-3">
                        {budgets.map((budget) => {
                            const { remaining, percentage, isOverBudget } = calculateProgress(budget);
                            const progressBarColor = isOverBudget ? 'bg-red-500' : 'bg-green-500';
                            const remainingColor = isOverBudget ? 'text-red-600 font-semibold' : 'text-green-600';

                            return (
                                <li key={budget.id} className={`p-3 rounded-md border flex flex-col gap-2 ${currentBudget?.id === budget.id ? 'bg-indigo-50 border-indigo-300' : 'bg-gray-50 border-gray-200'}`}>
                                    {/* Top Row: Name, Amount, Dates, Actions */}
                                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                                        <div className="flex-grow">
                                            <span className={`font-medium ${budget.category_id ? 'text-gray-800' : 'text-indigo-700 font-semibold'}`}>
