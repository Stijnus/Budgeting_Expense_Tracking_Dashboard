...
export default function HouseholdManager() {
  ...
  }>({}); // Errors per household for member actions
  const [isCreating, setIsCreating] = useState(false); // For create form button
  const [isAddingMember, setIsAddingMember] = useState<{
    [key: string]: boolean;
  }>({}); // Loading state per household for adding member
  const [isUpdatingRole, setIsUpdatingRole] = useState<{
    [key: string]: boolean;
  }>({}); // Loading state for role updates { householdId_userId: boolean }
  const [newHouseholdName, setNewHouseholdName] = useState("");
  const [userId, setUserId] = useState<string | null>(null);
  const [userRoles, setUserRoles] = useState<{ [householdId: string]: Role }>(
    {}
  ); // Store current user's role per household
  const [showAddMemberInput, setShowAddMemberInput] = useState<string | null>(
    null
  ); // ID of household to show input for
  ...
  const fetchHouseholds = useCallback(
    async (showLoadingIndicator = true) => {
      ...
      setMemberError({});
      try {
        // First, fetch just the household IDs and roles
        ...
        if (!memberEntries || memberEntries.length === 0) {
          console.log(
            "HouseholdManager: User is not a member of any households."
          );
          setHouseholds([]);
          setUserRoles({});
        ...
        memberEntries.forEach((m) => {
          rolesMap[m.household_id] = m.role as Role;
        });
        setUserRoles(rolesMap);
        console.log("HouseholdManager: User roles:", rolesMap);

        // Then fetch households with basic info
        ...
        // Finally, fetch members for each household
  ...
  const handleCreateHousehold = async (
    ...
    if (!newHouseholdName.trim() || !userId) {
      const errorMsg = !userId
        ? "User session error. Please refresh."
    ...
    try {
      const householdInsertData = {
        name: newHouseholdName.trim(),
        owner_id: userId,
      };
      console.log(
        "HouseholdManager: Inserting household with data:",
        householdInsertData
      );
      ...
      if (createError) {
        console.error(
          "HouseholdManager: Error inserting household:",
          createError
        );
        if (createError.message.includes("RLS")) {
          throw new Error(
            `Household insert failed due to RLS. Check policy 'Allow household insert for owner'. Details: ${createError.message}`
          );
        }
        throw createError;
      }
      ...
      console.log(
        "HouseholdManager: Household created successfully with ID:",
        newHouseholdId
      );
      ...
      console.log(
        "HouseholdManager: Inserting owner member with data:",
        memberInsertData
      );
      ...
      if (memberError) {
        console.error(
          "HouseholdManager: Error inserting owner member:",
          memberError
        );
        if (memberError.message.includes("RLS")) {
          throw new Error(
            `Owner member insert failed due to RLS. Check policy 'Allow owner or admin to insert members'. Details: ${memberError.message}`
          );
        }
        console.warn(
          "HouseholdManager: Failed to add owner member, attempting to delete household..."
        );
        await supabase.from("households").delete().eq("id", newHouseholdId);
        console.log(
          "HouseholdManager: Cleanup successful, household deleted:",
          newHouseholdId
        );
        throw memberError;
      }
  ...
  const handleDeleteHousehold = async (
    ...
    if (!userId || userId !== ownerId) {
      alert("Only the owner can delete the household.");
      return;
    }
    if (
      !window.confirm(
        "Are you sure? This deletes the household and removes all members."
      )
    )
      return;
    setLoading(true);
    ...
    try {
      console.log(
        `HouseholdManager: Attempting to delete household ${householdId} by owner ${userId}`
      );
      ...
      if (error) {
        ...
        if (error.message.includes("RLS")) {
          throw new Error(
            `Household delete failed due to RLS. Check policy 'Allow household delete for owner'. Details: ${error.message}`
          );
        }
        throw error;
      }
      console.log(
        `HouseholdManager: Household ${householdId} deleted successfully.`
      );
      fetchHouseholds(false);
    } catch (err: any) {
  ...
  const handleLeaveHousehold = async (householdId: string, role: Role) => {
    if (role === "owner") {
      alert(
        "Owners cannot leave. Delete the household or transfer ownership (not implemented)."
      );
      return;
    }
    if (
      !userId ||
      !window.confirm("Are you sure you want to leave this household?")
    )
      return;
    setLoading(true);
    setError(null);
    try {
      console.log(
        `HouseholdManager: User ${userId} attempting to leave household ${householdId}`
      );
      const { error } = await supabase
        .from("household_members")
        .delete()
      ...
      if (error) {
        ...
        if (error.message.includes("RLS")) {
          throw new Error(
            `Leave household failed due to RLS. Check policy 'Allow member delete for self, owner, or admin'. Details: ${error.message}`
          );
        }
        throw error;
      }
      console.log(
        `HouseholdManager: User ${userId} left household ${householdId} successfully.`
      );
      fetchHouseholds(false);
    } catch (err: any) {
  ...
  const handleAddMemberSubmit = async (
    ...
    console.log(
      `HouseholdManager: Attempting to add member ${emailToAdd} to household ${householdId}`
    );
    if (!emailToAdd) {
      setMemberError((prev) => ({
        ...prev,
        [householdId]: "Email cannot be empty.",
      }));
      return;
    }
    if (!userId) {
      setMemberError((prev) => ({
        ...prev,
        [householdId]: "Current user session error.",
      }));
      return;
    }
    ...
    try {
      ...
      console.log(
        `HouseholdManager: Calling RPC get_user_id_by_email for ${emailToAdd}`
      );
      ...
      if (rpcError || !rpcData) {
        console.error(
          "HouseholdManager: RPC Error or user not found:",
          rpcError
        );
        throw new Error(
          `User with email ${emailToAdd} not found or RPC error.`
        );
      }
      userToAdd = { id: rpcData, email: emailToAdd };
      console.log(
        `HouseholdManager: RPC successful, found user ID: ${userToAdd.id} for ${emailToAdd}`
      );

      const household = households.find((h) => h.id === householdId);
      const isAlreadyMember = household?.members.some(
        (m) => m.user_id === userToAdd?.id
      );
      if (isAlreadyMember) {
      ...
      const memberInsertData = {
        ...
        role: "member",
      }; // Always add as 'member' initially
      console.log(
        `HouseholdManager: Inserting new member link with data:`,
        memberInsertData
      );
      ...
      if (insertError) {
        console.error(
          "HouseholdManager: Error inserting member link:",
          insertError
        );
        if (insertError.message.includes("RLS")) {
          throw new Error(
            `Add member failed due to RLS. Check policy 'Allow owner or admin to insert members'. Details: ${insertError.message}`
          );
        }
        if (insertError.code === "23505") {
          throw new Error(`${emailToAdd} is already a member.`);
        }
        throw insertError;
      }
      console.log(
        `HouseholdManager: Successfully added ${emailToAdd} to household ${householdId}`
      );
      setAddMemberEmail("");
      setShowAddMemberInput(null);
      fetchHouseholds(false);
      ...
      setMemberError((prev) => ({
        ...prev,
        [householdId]: `Failed: ${err.message}`,
      }));
    } finally {
  ...
  const handleRemoveMember = async (
    ...
    if (
      ...
      alert("Only owners or admins can remove members.");
      return;
    }
    ...
    try {
      console.log(
        `HouseholdManager: User ${userId} (Role: ${currentUserRole}) attempting to remove member ${memberUserId} from household ${householdId}`
      );
      const { error } = await supabase
        .from("household_members")
        .delete()
      ...
      if (error) {
        ...
        if (error.message.includes("RLS")) {
          throw new Error(
            `Remove member failed due to RLS. Check policy 'Allow member delete for self, owner, or admin'. Details: ${error.message}`
          );
        }
        throw error;
      }
      console.log(
        `HouseholdManager: Member ${memberUserId} removed from household ${householdId} successfully.`
      );
      fetchHouseholds(false);
    } catch (err: any) {
      ...
      setMemberError((prev) => ({
        ...prev,
        [householdId]: `Remove failed: ${err.message}`,
      }));
    }
  };
  ...
  const handleUpdateRole = async (
    ...
    try {
      console.log(
        `HouseholdManager: Attempting to change role of ${memberUserId} to ${newRole} in household ${householdId}`
      );
      const { error } = await supabase
        .from("household_members")
        .update({ role: newRole })
      ...
      if (error) {
        ...
        if (error.message.includes("RLS")) {
          throw new Error(
            `Role update failed due to RLS. Check policy 'Allow owner or admin to update member roles'. Details: ${error.message}`
          );
        }
        throw error;
      }
      console.log(
        `HouseholdManager: Role for ${memberUserId} updated to ${newRole} successfully.`
      );
      ...
      setMemberError((prev) => ({
        ...prev,
        [householdId]: `Role update failed: ${err.message}`,
      }));
    } finally {
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow">
      ...
      <form
        ...
        className="mb-6 p-4 border border-gray-200 rounded-md bg-gray-50 space-y-3"
      >
        ...
        <button
          type="submit"
          disabled={isCreating || loading || !newHouseholdName.trim()}
      ...
      <div className="min-h-[100px]">
        {loading ? (
          <div className="flex items-center justify-center py-6 text-gray-500">
            <Loader2 className="animate-spin h-6 w-6 mr-3" /> Loading
            households...
          </div>
          ...
          <div className="text-center py-6 px-4 text-gray-500">
            ...
            <p className="text-xs">
              Create one above or ask someone to invite you.
            </p>
          </div>
        ) : (
          <ul className="space-y-4">
            {households.map((h) => {
              ...
              return (
                <li
                  key={h.id}
                  className="p-4 rounded-md border border-gray-200 bg-gray-50 space-y-3"
                >
                  {/* Household Header */}
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    ...
                    <div className="flex items-center space-x-2 mt-2 sm:mt-0 flex-shrink-0">
                      {isOwner && (
                        <button
                          onClick={() =>
                            handleDeleteHousehold(h.id, h.owner_id)
                          }
                          disabled={loading || isCreating}
                          className="p-1 text-red-600 hover:text-red-800 rounded hover:bg-red-100 disabled:opacity-50 focus:outline-none focus:ring-1 focus:ring-red-400"
                      ...
                      {!isOwner && (
                        <button
                          onClick={() =>
                            handleLeaveHousehold(h.id, currentUserRole)
                          }
                          disabled={loading || isCreating}
                          className="p-1 text-orange-600 hover:text-orange-800 rounded hover:bg-orange-100 disabled:opacity-50 focus:outline-none focus:ring-1 focus:ring-orange-400"
                  ...
                  <div>
                    ...
                    {/* Member List */}
                    <ul className="space-y-1 text-sm max-h-48 overflow-y-auto pr-1 mb-3">
                      {" "}
                      ...
                      {h.members.map((member) => {
                        ...
                        const isRoleUpdating =
                          isUpdatingRole[roleUpdateLoadingKey];

                        return (
                          <li
                            key={member.user_id}
                            className="flex justify-between items-center p-1.5 bg-white rounded border border-gray-100 gap-2"
                          >
                            {/* Left side: Email and Role */}
                            <span
                              className="truncate flex-grow"
                              title={member.users?.email || "Loading email..."}
                            >
                              {member.users?.email || (
                                <Loader2
                                  size={12}
                              ...
                              {member.role === "admin" && (
                                <span className="ml-1.5 text-xs font-semibold text-purple-600">
                                  (Admin)
                                </span>
                              )}
                              {memberIsSelf && (
                    ...
                    {canManageMembers && (
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        {showAddMemberInput === h.id ? (
                          <form
                            onSubmit={(e) => handleAddMemberSubmit(e, h.id)}
                            className="space-y-2"
                          >
                            {/* ... (input form remains the same) ... */}
                            <label
                              htmlFor={`add-email-${h.id}`}
                              className="sr-only"
                            >
                              Member Email
                            </label>
                            ...
                            <div className="flex items-center justify-end space-x-2">
                              <button
                                ...
                                disabled={addingThisMember}
                                className="px-2 py-1 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 disabled:opacity-50"
                              >
                                Cancel
                              </button>
                              <button
                                type="submit"
                                disabled={
                                  addingThisMember || !addMemberEmail.trim()
                                }
                                className="px-2 py-1 text-xs font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-green-500 disabled:opacity-50 flex items-center justify-center min-w-[60px]"
                              >
                                {addingThisMember ? (
                                ...
                                {addingThisMember ? "Adding..." : "Add"}
                              </button>
                            </div>
                          </form>
                        ) : (
                          <button
                            onClick={() => handleShowAddInput(h.id)}
                            disabled={
                              loading || isCreating || !!showAddMemberInput
                            }
                            className="px-2 py-1 text-xs font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-green-500 disabled:opacity-50 flex items-center gap-1"
                          >
                    ...
                    {canManageMembers &&
                      ...
                      showAddMemberInput !== h.id && (
                        <p className="text-xs text-red-600 mt-2 flex items-center gap-1">
                          <AlertTriangle size={14} /> {householdMemberError}
                        </p>
                      )}
                  </div>
                </li>
